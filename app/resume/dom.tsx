...
export default function Dom () {
  const { rect, isCopied, isSelectBackWard, visible, startCopy } = useTextSelection()

  const styleContainer = useMemo(() => {
    if (!rect) {
      return undefined
    }
    const { x, top, height } = rect
    const translateX = Math.floor(x + ( isSelectBackWard ? - (65 + 5) : 5 ))
    const translateY = Math.round(top - ( (29 - height) / 2 ))
    return {
      transform: `translate3d(${translateX}px, ${translateY}px, 0)`
    }
  }, [rect, isSelectBackWard])
  ...
  return visible ? (
    <div
      ...
      onClick={() => startCopy()}
    >
      <PopoverCopy isCopied={isCopied} />
    </div>
  ) : null
}

function PopoverCopy (props: { isCopied: boolean }) {
  if (props.isCopied) {
    return (
      <>
...
function useTextSelection () {
  ...
  const [isSelectBackWard, setIsBackward] = useState<boolean | null>(null)
  ...
  useEffect(() => {
    if (visible && timerCopy.current) {
      window.clearTimeout(timerCopy.current)
    }
    setCopied(false)
  }, [visible])
